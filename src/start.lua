...
function start:enter()
  startgame = false
  flux.to(textopacity, 0.5, { 1 }):delay(0.5)
...
function start:draw()
  ...
  love.graphics.setColor(1, 1, 1, textopacity[1])
  ...
  printfshadow("Space and arrow keys.", 0, y + 180, width, 'center', textopacity[1])
end

function start:keypressed(key)
  if key == 'space' and not startgame then
    startgame = true
    flux.to(textopacity, 0.5, { 0 }):delay(0.3)
    for k, o in ipairs(tiles.ents) do
      if o.tweenpos then
        o.tweenpos:stop()
      end
